...
Keep state as close to the component needing it.

To share state move the hooks of a component state to the closest shared parent with the components that need that state.

Event sequence: lazy initializer->render->update DOM->clean layout effects->run layout effecs->browser paint screens->clean up effects->run effects
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=03%3A%20Lifting%20state&em=a%40b.c).
