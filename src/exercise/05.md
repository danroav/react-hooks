...
useEffect returns a cleaner callback to dispose elements or operations in the useEffect area

useEffect has an additional parameter to specify the dependencies on when to run the effect, if not defined it runs every time, if defined and empty runs only once, if defined and containing arguments then it runs on every change in the arguments
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=a%40b.c).
