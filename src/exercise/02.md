...
useState hook can receive a function to initialize the state only on first render

useEffect hook can specify an array of dependency values to check in order to execute

localStorage.getItem returns null if there is no key or value associated with the key

a custom hook function name must start with use and usually is composed of other hooks
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=a%40b.c).
